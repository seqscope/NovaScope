...
## required parameters
parser.add_argument("--sdge", type=str, required=True, help="Input (sttools2 output) SDGE directory")
parser.add_argument("--out", type=str, required=True, help="Output file prefix")
## layout required - one or the other
...
## scaling parameters
parser.add_argument("--scale", type=float, default=20.0, help="Scale each color to have the same mean intensity (0 indicates no scaling)")
...
parser.add_argument("--transpose", default=False, action='store_true', help="Transpose the image -- x is width and y is height")

args = parser.parse_args()

## assuming x/y ranges are the same across tile, empirically determine max x/y values
...
for r in range(nrows):
    for c in range(ncols):
        ...
        ftrf = f"{args.sdge}/{lanes[r][c]}/{tiles[r][c]}/features.tsv.gz"
        ...
        with gzip.open(ftrf,'rt',encoding='utf-8') as fh:
            for line in fh:
                ...
                gidx = int(toks[2])-1
                rgbw = [0, 0, 0]  ## weight for RGB colors
                rgbi = [0, 0, 0]  ## index for RGB colors
                for i in range(3):
                    ## if gene name is _all, include all genes
                ...
                if ( rgbw[0] + rgbw[1] + rgbw[2] > 0 ):
                    colDict[toks[2]] = [rgbw, rgbi] ## make indices

        with gzip.open(f"{args.sdge}/{lanes[r][c]}/{tiles[r][c]}/barcodes.tsv.gz",'rt',encoding='utf-8') as bh:
            with gzip.open(f"{args.sdge}/{lanes[r][c]}/{tiles[r][c]}/matrix.mtx.gz",'rt',encoding='utf-8') as mh:
                ...
                update_xyrange(xyrange, int(btoks[5]), int(btoks[6]))
                for i in range(nlines):
                    mtoks = mh.readline().split()            ## [iftr, ibcd, gene, genefull, spliced, unspliced, ambiguous]
                    while ( int(btoks[1]) < int(mtoks[1]) ): ## match ibcd from barcodes and mtx
                        btoks = bh.readline().split('\t')
                        update_xyrange(xyrange, int(btoks[5]), int(btoks[6]))                        
                    if ( btoks[1] != mtoks[1] ):
                    ...
                    if ( mtoks[0] in colDict ): ## gene has to be counted
                        ...
                        (lane, tile, x, y) = (btoks[3],btoks[4],int(btoks[5]),int(btoks[6]))
                        ## cnts = [int(z) for z in btoks[7].split(',')]
                        cnts = [int(z) for z in mtoks[2:]]
                        xbin = x // args.res
                        ybin = y // args.res
...
if args.transpose:
    data = np.flip(data.transpose((1,0,2)),axis=1)
...
if args.tif:
    img.save(args.out if args.out.endswith(".tif") else f"{args.out}.tif")
else:
    img.save(args.out if args.out.endswith(".png") else f"{args.out}.png")
