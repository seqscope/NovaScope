...
from rule_general_novascope import assign_resource_for_align, get_envmodules_for_rule, get_skip_sbcd, find_major_axis, get_python
...
log_a_separator()
debug=config.get("debug", False) # if debug, report details for each output file
quite=config.get("quite", False) # if quite, skip the details for processing config

# - env (store env setting into config)
config = read_config_for_ini(config, job_dir, smk_dir, silent=False)
...
logging.info(f" - Output root directory: {main_root}")
...
request = check_request(input_request=config.get("request", ["sge-per-run"]), 
                        valid_options=["sbcd-per-flowcell", "sbcdlo-per-flowcell", "sbcd-per-chip", "smatch-per-chip", "align-per-run", "sge-per-run", "histology-per-run", 
...
seq1_id = read_config_for_seq1(config, silent=False)
sc2seq1 = {chip:seq1_id} 

# per-chip: (smatch-per-chip, sbcd-per-chip (and above))
df_seq2 = read_config_for_seq2(config, silent=False)

# per-unit or per-run:
run_id, rid2seq2 = read_config_for_runid(config, df_seq2, silent=False)
print(run_id)
config["input"]["run_id"] = run_id
print

# unit ID: to distinguish the default sge and the sge with manual boundary filtering.
unit_id, unit_ann, boundary = read_config_for_unitid(config, silent=False)
...
df_sge, sgevisual_id2params, rid2sgevisual_id = read_config_for_sgevisual(config, df_run, silent=False)

# hist
df_hist = read_config_for_hist(config, df_run, silent=False)
...
output_filename_conditions.extend(outfnlist_by_seg(main_dirs, df_seg))   

requested_files = list_outputfn_by_request(output_filename_conditions, request, debug)
