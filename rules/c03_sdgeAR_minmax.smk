rule c03_sdgeAR_minmax:
    ...
    output:
        sdgeAR_xyrange_raw       = os.path.join(main_dirs["analysis"], "{run_id}", "{unit_id}", "preprocess", "{unit_id}.{solo_feature}.raw.coordinate_minmax.tsv"),
    params:
        # module
    ...
    run:
        ...
        gzip -cd {input.sdgeAR_transcript} | \
            ...
            awk 'BEGIN {{FS=OFS="\t"; min1 = "undef"; max1 = "undef"; min2 = "undef"; max2 = "undef";}} {{if (NR == 2 || $1 < min1) min1 = $1; if (NR == 2 || $1 > max1) max1 = $1; if (NR == 2 || $2 < min2) min2 = $2; if (NR == 2 || $2 > max2) max2 = $2;}} END {{print "xmin", min1; print "xmax", max1; print "ymin", min2; print "ymax", max2;}}' > {output.sdgeAR_xyrange_raw}
