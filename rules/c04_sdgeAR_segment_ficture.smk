rule c04_sdgeAR_segment_ficture:
    input:
        ...
        sdgeAR_bd_qc         = lambda wildcards: os.path.join(main_dirs["analysis"], "{run_id}", "{unit_id}", "preprocess", "{unit_id}.{solo_feature}.{sge_qc}.boundary.geojson") if wildcards.sge_qc == "filtered" else "",
    output:
