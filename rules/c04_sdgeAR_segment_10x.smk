rule c04_sdgeAR_segment_10x:
    input:
        ...
        transcript_in   = lambda wildcards: os.path.join(main_dirs["analysis"], "{run_id}", "{unit_id}", "preprocess", ("{unit_id}.transcripts.tsv.gz" if wildcards.sge_qc=="raw" else "{unit_id}."+wildcards.sge_qc+".transcripts.tsv.gz")),
